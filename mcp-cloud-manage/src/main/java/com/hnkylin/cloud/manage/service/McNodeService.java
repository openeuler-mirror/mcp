...
public class McNodeService {
        ...
        if (!clusterDoList.isEmpty()) {
            clusterDoList.forEach(item -> {
                ...
                if (!mcNodeList.contains(mcLeaderUrl)) {
                    mcLeaderNodeCacheService.vDelete(CloudManageRedisConstant.MC_LEADER_URL_KEY + clusterId);
                }
                if (!mcNodeList.isEmpty()) {
    ...
    public String getMcLeaderUrlFromCache(Integer clusterId) {
        Object mcLeaderUrl = mcLeaderNodeCacheService.vGet(CloudManageRedisConstant.MC_LEADER_URL_KEY + clusterId);
    ...
    public void setMcLeaderUrlToCache(String mcLeaderUrl, Integer clusterId) {
        mcLeaderNodeCacheService.vSet(CloudManageRedisConstant.MC_LEADER_URL_KEY + clusterId, mcLeaderUrl);
     ...
     * @param clusterId
     */
    public void deleteMcNodeListToCache(Integer clusterId) {
        mcLeaderNodeCacheService.vDelete(CloudManageRedisConstant.CLOUD_MANAGE_MC_NODE_LIST_KEY + clusterId);
    }
    ...
    public void setMcNodeListToCache(List<String> mcNodeList, Integer clusterId) {
        mcLeaderNodeCacheService.vSet(CloudManageRedisConstant.CLOUD_MANAGE_MC_NODE_LIST_KEY + clusterId, mcNodeList);
    }
    ...
    public List<String> getMcNodeListFromCache(Integer clusterId) {
        Object mcNodeList =
                mcLeaderNodeCacheService.vGet(CloudManageRedisConstant.CLOUD_MANAGE_MC_NODE_LIST_KEY + clusterId);
